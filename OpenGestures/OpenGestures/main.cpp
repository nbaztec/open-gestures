 ...
 * @summary The application uses finger count detection and adds stable triggering
 *			mechanism to initiate applications based on the number of fingers
 *			The associated applications can be set in 'config.ini' file used with
 *			this application
 * @author Nisheeth Barthwal
 * @author Aniket Tatipamula (http://anikettatipamula.blogspot.in/)
 * @link http://anikettatipamula.blogspot.in/2012/02/hand-gesture-using-opencv.html
 * @license Creative Commons Licence (http://creativecommons.org/licenses/by/3.0/)
 */
...
void read_config()
{
	...
	if(config)
	{
		...
		sscanf(line, "stable_seconds = %s", line);
		stable_seconds = atoi(line);

		fgets(line, 255, config);
		sscanf(line, "detect_1 = %s", actions[0]);		
		fgets(line, 255, config);
		sscanf(line, "detect_2 = %s", actions[1]);
		fgets(line, 255, config);
		sscanf(line, "detect_3 = %s", actions[2]);
		fgets(line, 255, config);
		sscanf(line, "detect_4 = %s", actions[3]);
		fgets(line, 255, config);
		sscanf(line, "detect_5 = %s", actions[4]);
...
int main(int argc, char** argv)
{
	...
	printf("Learning first %d frames\n", nframesToLearnBG);
	...
	for(;;)
	{
		...
		if( rawImage )
		{
			...
			if( nframes-1 >= nframesToLearnBG  )
			{
				...
				cvSaveImage("Image.jpg", ImaskCodeBookCC);
...
void  detect(IplImage* img_8uc1,IplImage* img_8uc3) {	
	...
	if(Nc>0)
	{
		...
		if(areamax>5000)
		{
			...
			p2.x = r.x + r.width;
			p2.y = r.y;
			cvRectangle(img_8uc1, p1, p2, CV_RGB(255, 0, 0));
			...
			{  
				...
				{   //printf(" defect depth for defect %d %f \n",i,defectArray[i].depth);
					//if(nomdef > 1 && i == nomdef-2)
					//	printf("[%d] : %d, %d\n", i, defectArray[i].depth_point->x, defectArray[i].depth_point->y);
					cvLine(img_8uc3, *(defectArray[i].start), *(defectArray[i].depth_point),CV_RGB(255,255,0),1, CV_AA, 0 );  
					cvCircle( img_8uc3, *(defectArray[i].depth_point), 5, CV_RGB(colors[i][0],colors[i][1],colors[i][2]), 2, 8,0);  
					cvCircle( img_8uc3, *(defectArray[i].start), 2, CV_RGB(colors[i][0],colors[i][1],colors[i][2]), 2, 8,0);  
				...
				if((nomdef-1) != last_number)
				{
					//printf("Triggered: %d vs %d : ", last_number, nomdef-1);
					last_stable = time(NULL);
					last_number = nomdef-1;
					stable = 0;
					triggered = 1;				
					//printf("Triggered: %d vs %d at %d\n", last_number, nomdef-1, time(NULL));
				}
				else
				{
					//printf("Stable %d? %d\n", last_number, time(NULL));
					stable = time(NULL) - last_stable >= stable_seconds;
				}

				if(triggered && stable)
				{
					triggered = 0; // bounce back
					printf("Detected: %d\n", last_number);
					switch(last_number)
					{
					case 1:
						ShellExecuteA(NULL, "open", actions[0], "", "", SW_SHOWNORMAL);
						break;
					case 2:
						ShellExecuteA(NULL, "open", actions[1], "", "", SW_SHOWNORMAL);
						break;
					case 3:
						ShellExecuteA(NULL, "open", actions[2], "", "", SW_SHOWNORMAL);
						break;
					case 4:
						ShellExecuteA(NULL, "open", actions[3], "", "", SW_SHOWNORMAL);
						break;
					case 5:
						ShellExecuteA(NULL, "open", actions[4], "", "", SW_SHOWNORMAL);
						break;
					}
				}
			} 
